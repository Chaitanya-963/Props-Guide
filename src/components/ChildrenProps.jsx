function Card({ children, title, color = "blue", isDark = true }) {
  ...
  return (
    <div
      className={`border-l-4 ${colorClasses[color] || colorClasses.blue} p-4 sm:p-6 rounded-r-xl rounded-l-xs shadow-md transition-all duration-300 backdrop-blur-xs flex flex-col justify-between h-full`}
    >
      <div>
        {title && (
          <h3
            className={`text-base sm:text-lg font-bold mb-2.5 sm:mb-3 tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}
          >
            {title}
          </h3>
        )}
        <div
          className={`text-xs sm:text-sm leading-relaxed ${isDark ? "text-slate-300" : "text-slate-700"}`}
        >
          {children}
        </div>
      </div>
...
function Container({ children, layout = "vertical" }) {
  const layoutClasses = {
    // Made horizontal layout dynamically adjust column weights safely on smartphones
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <p
          className={`text-sm sm:text-base leading-relaxed mb-6 sm:mb-8 max-w-3xl ${isDark ? "text-slate-400" : "text-slate-600"}`}
        >
          The{" "}
        ...
        <div className="space-y-4">
          <h3
            className={`text-xs sm:text-sm font-semibold tracking-wide uppercase ${isDark ? "text-slate-400" : "text-slate-500"} mb-3 sm:mb-4`}
          >
          ...
          <Container layout="grid">
            ...
            <Card title="Company Statistics" color="green" isDark={isDark}>
              ...
              <button className="w-full px-4 py-2 text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg transition-colors cursor-pointer shadow-sm active:scale-[0.98]">
                Connect with me!
              </button>
            </Card>
            ...
            <Card title="System Update" color="purple" isDark={isDark}>
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                <span className="text-[10px] sm:text-[11px] font-mono px-2 py-0.5 rounded bg-purple-500/20 text-purple-400 border border-purple-500/30 whitespace-nowrap">
                  v2.4.1-stable
                </span>
                <span className="text-[10px] sm:text-[11px] font-mono px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 whitespace-nowrap">
                  SHA-8f2c7a
                </span>
              </div>
            </Card>
            ...
            <Card title="API Connection Failure" color="red" isDark={isDark}>
              ...
              <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-rose-400 mt-auto">
            ...
            <Card title="Subscription Notice" color="amber" isDark={isDark}>
              ...
              <button
                className={`w-full text-center px-3 py-1.5 text-xs font-semibold rounded-md border transition-all mt-auto active:scale-[0.98] ${
                  isDark
